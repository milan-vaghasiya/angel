$(document).ready(function(){
    ...
    $(document).on('click', '.saveItem', function () {
		...
		if($("#brand_id").val() != ""){
            formData.brand_name = $("#brand_id :selected").data('brand_name');
        }
	...
	$(document).on('change','#item_id',function(e){
		...
		$.ajax({
			url:base_url + "salesQuotation/getRevisionList",
			type:'post',
...
function AddRow(data) {
	...
	var brandInput = $("<input/>", { type: "hidden", name: "itemData["+itemCount+"][brand_id]", value: data.brand_id });
	cell = $(row.insertCell(-1));
	cell.html(data.brand_name);
    cell.append(brandInput);
...
function getBrandList(postData){
	$.ajax({
		url : base_url + '/selectOption/getBrandList',
		type : 'post',
		data : postData,
		dataType : 'json',
	}).done(function(response){
		var brandList = response.data.brandList;
		var html = '<option value="">Select Brand</option>';
		if(!$.isEmptyObject(brandList)){
			$.each(brandList,function(index,row){
