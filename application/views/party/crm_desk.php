			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<div class="float-end">
							<?php
								$addParam = "{'postData' : {'party_type' : '2','party_category' : '1'}, 'modal_id' : 'bs-right-lg-modal', 'call_function' : 'addParty', 'form_id' : 'addLead', 'title' : 'Add Lead', 'controller' : 'parties', 'js_store_fn' : 'saveLead'}";

								$excelParam = "{'postData':{'party_category' :1,'party_type' : '2'},'modal_id' : 'bs-right-xl-modal', 'call_function':'uploadPartyExcel','fnsave':'saveUploadedExcel', 'form_id' : 'uploadParty', 'title' : 'Add Party','button':'close'}";
							
								$assignExt = "{'postData':{'id':''},'fnsave':'saveBulkExecutive','modal_id':'bs-right-xl-modal','form_id':'addBulkExecutive','call_function':'addBulkExecutive', 'title' : 'Asign Executive','js_store_fn':'saveBulkExecutive'}";
							?>

							<button type="button" class="btn waves-effect waves-light btn-dark btn-sm float-right permission-write press-add-btn" onclick="modalAction(<?=$addParam?>);"><i class="fa fa-plus"></i> Add Lead</button>
						
							<a href="javascript:void(0)" class="btn waves-effect waves-light btn-success btn-sm float-right permission-modify mr-2" onclick="modalAction(<?=$excelParam?>);" datatip="Add Lead Excel" flow="down"><i class="fas fa-file-excel"></i> Upload Excel</a>
							
							<a href="javascript:void(0)" class="btn waves-effect waves-light btn-dribbble btn-sm float-right permission-modify mr-2" onclick="modalAction(<?=$assignExt?>);" data-msg="Bulk Executive" flow="down"><i class="fa fa-plus"></i> Asign Executive</a>
							...
							<?php
								if(!empty($stageList)){
									foreach($stageList as $row) {
										if($row->sequence != 1){
											$active="";
											if($row->stage_type == "NEW"){$active = 'active';}
											?>
											<li class="nav-item"> <button data-filter=".<?=$row->lead_stage?>_lead" class="buttonFilter stageFilter mr-1 btn btn-outline-info <?=$active?>" data-lead_stage="<?=$row->lead_stage?>" data-postdata='{"lead_stage":"<?=$row->lead_stage?>"}' data-toggle="tab"><?=$row->stage_type?></button> </li>
											<?php
										}
									}
								}
										...
										<div class="cd-features visually-hidden">
											<div class="d-none d-sm-inline-block">
												...
												<div class="dropdown-menu dropdown-menu-end" aria-labelledby="dLabel1" style="">
													...
													<a href="javascript:void(0)" class="text-primary permission-write dropdown-item salesData" data-button="both" onclick="window.location.href='<?=base_url('salesEnquiry/addEnquiry')?>'"><i class="far fa-question-circle align-text-bottom"></i> Sales Enquiry</a>

													<div class="dropdown-divider mb-0"></div>
													<a href="javascript:void(0)" class="text-info permission-write dropdown-item" data-button="both" onclick="window.location.href='<?=base_url('salesQuotation/addQuotation')?>'"><i class="far fa-file-alt align-text-bottom"></i> Sales Quotation</a>

													<div class="dropdown-divider mb-0"></div>
													<a href="javascript:void(0)" class="text-success permission-write dropdown-item" data-button="both" onclick="window.location.href='<?=base_url('salesOrders/addOrder')?>'"><i class="mdi mdi-cart-plus align-text-bottom"></i> Sales Order</a>
...
$(document).ready(function(){
    ...
    $(document).on('change','#source',function(){ $(".stageFilter.active").trigger("click"); });
    ...
    $(document).on('click','.saveFollowups',function(){
		...
		if(notes != ''){
			$.ajax({
				...
				data: {party_id:party_id, lead_id:lead_id, notes:notes,lead_stage:3,id:''},
				type: "POST",
				global:false,
				dataType:"json",
...
function saveBulkExecutive(postData){
	$(".csearch").val('');
