...
class ItemCategoryModel extends MasterModel{
	...
	public function getDTRows($data){
        ...
        $data['select'] = "item_category.*, IFNULL(pc.category_name,'NA') as parent_category_name, 
		(CASE WHEN item_category.final_category = 1 THEN 'YES' ELSE 'NO' END) as is_final_text,
		(CASE WHEN item_category.batch_stock = 1 THEN 'Batch Wise' WHEN item_category.batch_stock = 2 THEN 'Serial Wise' ELSE 'None' END) as stock_type_text,
		(CASE WHEN item_category.is_return = 1 THEN 'YES' ELSE 'NO' END) as is_returnable_text"; 
		...
		$data['order_by']['category_type'] = 'ASC';
        ...
        $data['searchCol'][] = "if(item_category.final_category = 1, 'YES', 'NO')";
        $data['searchCol'][] = "if(item_category.is_return = 1,'YES','NO')";
    ...
    public function getCategoryList($data=array()){
		...
		$queryData['order_by']['category_type'] = 'ASC';
